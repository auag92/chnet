"""
https://www.ics.uci.edu/~haoyum3/papers/slides_icivc.pdf
"""
...
class LSTM_MMUnet(nn.Module):
    def __init__(self, input_nc=1, output_nc=5, ngf=32, temporal=3):
        ...
        self.mmout = nn.Conv2d(ngf, output_nc, kernel_size=1, stride=1, padding=0)
        self.out = nn.Conv2d(ngf, output_nc, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        for t in range(self.temporal):
            ...
            mm_last = self.unet(im_t) # bz * 32 * 240 * 240
            out_t = self.mmout(mm_last) # bz * 5 * 240 * 240
            
            mm_output.append(torch.tanh(out_t))
            ...
            output.append(torch.tanh(out_t))
            
            im_t = out_t
            
#         return torch.stack(mm_output, dim=1)   
