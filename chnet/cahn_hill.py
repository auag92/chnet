...
def ch_run_torch(x_data, gamma = 1., dx = 0.25, dt = 0.001, sim_step = 1, device = torch.device("cuda")):
    ...
    ksq = torch.from_numpy(ksq).double().to(device)
    ...
    response = torch.zeros(list(x_data.shape)+[2,], dtype=torch.double, device=device)
    response[...,0] = torch.from_numpy(x_data).double().to(device)
    Fy = fft(response)
